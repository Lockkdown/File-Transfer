# Ứng dụng chuyển tập tin (Java Client/Server File Transfer)

## 📚 Đồ án môn học: Lập trình mạng máy tính

### Thông tin nhóm

| STT | Họ và tên | MSSV |
|:---:|-----------|------|
| 1 | Trương Hoàng Phúc | 2280602486 |
| 2 | Ngô Xuân Hạo | 2280600867 |

---

## 📝 Mô tả dự án

Ứng dụng truyền file qua **TCP sockets** với các tính năng:
- Đăng ký / Đăng nhập / Quên mật khẩu
- Upload / Download file
- Chia sẻ file với quyền (OWNER / EDIT / VIEW)
- Quản lý version file
- Demo với Wireshark capture

---

## 🏗️ Kiến trúc hệ thống

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                           DRIVE-LITE ARCHITECTURE                           │
└─────────────────────────────────────────────────────────────────────────────┘

    ┌──────────────┐                                    ┌──────────────┐
    │   Client A   │                                    │   Client B   │
    │   (JavaFX)   │                                    │   (JavaFX)   │
    └──────┬───────┘                                    └──────┬───────┘
           │                                                   │
           │  TCP Socket                         TCP Socket    │
           │  Port 9000                          Port 9000     │
           │                                                   │
           ▼                                                   ▼
    ┌─────────────────────────────────────────────────────────────────┐
    │                        JAVA TCP SERVER                          │
    │  ┌─────────────┐  ┌─────────────┐  ┌─────────────────────────┐  │
    │  │   Accept    │  │   Client    │  │      Request            │  │
    │  │    Loop     │─▶│   Handler  │─▶│      Dispatcher         │  │
    │  │             │  │  (Thread)   │  │                         │  │
    │  └─────────────┘  └─────────────┘  └───────────┬─────────────┘  │
    │                                                │                │
    │         ┌──────────────────────────────────────┼────────┐       │
    │         ▼                  ▼                   ▼        ▼       │
    │  ┌────────────┐    ┌────────────┐    ┌──────────┐ ┌─────────┐   │
    │  │    Auth    │    │    File    │    │  Share   │ │ Version │   │
    │  │  Handlers  │    │  Handlers  │    │ Handlers │ │ Handler │   │
    │  └─────┬──────┘    └─────┬──────┘    └────┬─────┘ └────┬────┘   │
    │        │                 │                │            │        │
    │        └─────────────────┴────────────────┴────────────┘        │
    │                              │                                  │
    │                              ▼                                  │
    │                    ┌─────────────────┐                          │
    │                    │   Repository    │                          │
    │                    │     Layer       │                          │
    │                    └────────┬────────┘                          │
    └─────────────────────────────┼───────────────────────────────────┘
                                  │
                    ┌─────────────┴─────────────┐
                    │                           │
                    ▼                           ▼
           ┌─────────────────┐        ┌─────────────────┐
           │   SQL Server    │        │   File System   │
           │   (Metadata)    │        │   (Storage)     │
           │                 │        │                 │
           │  - Users        │        │  ./storage/     │
           │  - Sessions     │        │    ├── file1/   │
           │  - Files        │        │    │   └── v1   │
           │  - Permissions  │        │    └── file2/   │
           │  - Versions     │        │        └── v1   │
           └─────────────────┘        └─────────────────┘
```

### Luồng xử lý chính

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                         PROTOCOL FRAME FORMAT                               │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│   ┌──────────────┬─────────────────────────────────────────────────────┐    │
│   │  4 bytes     │              N bytes                                │    │
│   │  (length)    │           (JSON payload)                            │    │
│   │  Big-endian  │                                                     │    │
│   └──────────────┴─────────────────────────────────────────────────────┘    │
│                                                                             │
│   Ví dụ: LOGIN request                                                      │
│   ┌──────────────┬─────────────────────────────────────────────────────┐    │
│   │  00 00 00 45 │ {"type":"LOGIN","data":{"email":"a@b.com",...}}     │    │
│   └──────────────┴─────────────────────────────────────────────────────┘    │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```

### Upload Flow

```
    Client                                Server
      │                                     │
      │  1. UPLOAD_BEGIN                    │
      │  {fileName, fileSize, sha256}       │
      │────────────────────────────────────▶│
      │                                     │
      │  2. READY                           │
      │◀────────────────────────────────────│
      │                                     │
      │  3. Raw file bytes                  │
      │  (exactly fileSize bytes)           │
      │────────────────────────────────────▶│
      │                                     │
      │  4. UPLOAD_OK                       │
      │  {fileId, versionNumber}            │
      │◀────────────────────────────────────│
      │                                     │
```

### Permission Model (ACL)

```
┌─────────────────────────────────────────────────────────────────┐
│                    PERMISSION MATRIX                            │
├─────────────┬─────────┬─────────┬─────────┬─────────┬──────────┤
│  Action     │  OWNER  │  EDIT   │  VIEW   │  NONE   │          │
├─────────────┼─────────┼─────────┼─────────┼─────────┼──────────┤
│  View/List  │   ✅    │   ✅    │   ✅    │   ❌    │          │
│  Download   │   ✅    │   ✅    │   ✅    │   ❌    │          │
│  Upload Ver │   ✅    │   ✅    │   ❌    │   ❌    │          │
│  Share      │   ✅    │   ❌    │   ❌    │   ❌    │          │
│  Revoke     │   ✅    │   ❌    │   ❌    │   ❌    │          │
└─────────────┴─────────┴─────────┴─────────┴─────────┴──────────┘
```

---

## ⚙️ Prerequisites (Yêu cầu)

- **Java 17+** (khuyến nghị JDK 21)
- **Maven 3.6+**
- **SQL Server 2019+** (hoặc SQL Server Express)
- **SQL Server Management Studio (SSMS)** để chạy migration script

### Kiểm tra version

```powershell
java -version    # Phải >= 17
mvn -version     # Phải >= 3.6
```

## Setup

### 1. Clone và cấu hình

```powershell
# Clone repo
git clone <repo-url>
cd File_Transfer

# Copy file cấu hình mẫu
copy .env.example .env

# Sửa .env với thông tin SQL Server của bạn
notepad .env
```

### 2. Tạo Database

Mở SSMS, kết nối đến SQL Server, chạy:

```sql
CREATE DATABASE FileTransfer;
GO
```

### 3. Chạy Migration Script

Trong SSMS:
1. Chọn database `FileTransfer`
2. File → Open → `src/server/src/main/resources/db/V1__init_schema.sql`
3. Execute (F5)

### 4. Build Project

```powershell
cd src
mvn clean install
```

## Run

### Start Server

```powershell
cd src/server
mvn exec:java "-Dexec.mainClass=com.drivelite.server.ServerMain"
```

### Start Client

```powershell
cd src/client
mvn javafx:run
```

## Project Structure

```
File_Transfer/
├── .env.example        # Cấu hình mẫu
├── src/
│   ├── pom.xml         # Parent POM
│   ├── common/         # Shared code (protocol, DTOs)
│   ├── server/         # TCP Server + DB
│   └── client/         # JavaFX Client
└── README.md
```
## 📖 Tài liệu tham khảo

- [Oracle Java Networking Tutorial](https://docs.oracle.com/javase/tutorial/networking/)
- [JavaFX Documentation](https://openjfx.io/openjfx-docs/)
- [Microsoft JDBC Driver for SQL Server](https://learn.microsoft.com/en-us/sql/connect/jdbc/)
- [Wireshark User Guide](https://www.wireshark.org/docs/wsug_html/)

---
